...
# Supervised-only Trainer from UBTeacher v1 #Sort of HACKED
...
class UBRCNNTeacherTrainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        mapper = DatasetMapperTwoCropSeparate(cfg, True)
        #we define a custom mapper in the below function (build.py)
        return build_detection_semisup_train_loader_two_crops(cfg, mapper)
    ...
    def run_step_full_semisup(self):
        ...
        assert self.model.training, "[UBTeacherTrainer] model was changed to eval mode!"
        start = time.perf_counter()
        data = next(self._trainer._data_loader_iter) #creates the dataloader and uses it to generate
        # data_q and data_k from different augmentations (q:strong, k:weak)
        # label_strong, label_weak, unlabed_strong, unlabled_weak
        label_data_q, label_data_k, unlabel_data_q, unlabel_data_k = data
        data_time = time.perf_counter() - start
