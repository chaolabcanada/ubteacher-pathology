...
from tissue_finder import TissueFinder
...
class GenerateCam:
    def __init__(self, model: object, inputs: Dict, target_class: int, target_layers: List) -> None:
        self.model = model
        self.inputs = inputs
        self.input_image = np.transpose(self.inputs[0]['image'].data.numpy(), [1, 2, 0]) # (H, W, C)
